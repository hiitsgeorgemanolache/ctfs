...
The file is “hidden” because of the **.** at the beginning (Unix convention to keep the workspace cleaner), but `ls` has `-a` flag which outputs all files in the directory, as it doesn not ignore entries starting with **.**.
